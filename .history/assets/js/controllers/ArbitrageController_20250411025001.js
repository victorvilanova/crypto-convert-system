...
export default class ArbitrageController {
  ...
  async loadArbitrageData() {
    ...
    try {
      ...
      // Busca oportunidades de arbitragem usando o serviço
      const [triangularOpportunities, exchangeOpportunities] = await Promise.all([
        this.arbitrageService.getTriangularArbitrageOpportunities(),
        this.arbitrageService.getExchangeArbitrageOpportunities()
      ]);
      
      // Atualiza a UI com os dados obtidos
      this.updateArbitrageUI(triangularOpportunities, exchangeOpportunities);
      
    } catch (error) {
      console.error('Erro ao carregar dados de arbitragem:', error);
      this.showNotification('Ocorreu um erro ao carregar os dados de arbitragem.', 'error');
    } finally {
      this.isLoading = false;
