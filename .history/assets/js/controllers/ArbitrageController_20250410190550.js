...
export default class ArbitrageController {
  ...
  displayTriangularArbitrage(opportunities) {
    ...
    if (opportunities.length === 0) {
      ...
      cell.colSpan = 4;
      cell.textContent = 'Nenhuma oportunidade de arbitragem triangular encontrada.';
    ...
    opportunities.forEach(opportunity => {
      ...
      stepsCell.innerHTML = opportunity.steps.map(step => 
        `${step.from} → ${step.to}: ${step.rate}`
      ).join('<br>');
  ...
  displayExchangeArbitrage(opportunities) {
    ...
    if (opportunities.length === 0) {
      ...
      const cell = document.createElement('td');
      cell.colSpan = 5;
      cell.textContent = 'Nenhuma oportunidade de arbitragem entre exchanges encontrada.';
      cell.className = 'text-center p-3';
      row.appendChild(cell);
  ...
  simulateArbitrageExecution(opportunity) {
    ...
    alert(`Simulando execução da oportunidade de arbitragem:\n${JSON.stringify(opportunity, null, 2)}`);
