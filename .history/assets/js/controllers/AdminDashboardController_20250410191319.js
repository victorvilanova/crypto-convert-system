...
export default class AdminDashboardController {
  ...
  authenticate(username, password) {
    try {
      ...
      if (result.success) {
        ...
        if (this.adminUsernameSpan) {
          this.adminUsernameSpan.textContent = username;
        }
        ...
        if (this.loginError) {
          this.loginError.textContent = result.message || 'Credenciais inválidas. Tente novamente.';
      ...
      if (this.loginError) {
        this.loginError.textContent = 'Ocorreu um erro durante a autenticação. Tente novamente.';
        this.loginError.style.display = 'block';
      }
    }
  }
  ...
  async loadDashboardData() {
    ...
    try {
      ...
      this.displayError('Não foi possível carregar os dados. Tente novamente mais tarde.');
    }
  }
  ...
  updateDashboardStats(triangularOpportunities, exchangeOpportunities) {
    ...
    const totalOpportunities = triangularOpportunities.length + exchangeOpportunities.length;
