...
export function initializeRatesModule() {
  ...
  document.getElementById('manual-refresh').addEventListener('click', fetchCurrentRates);
...
async function fetchCurrentRates() {
  try {
    ...
    document.getElementById('rates-loader').classList.remove('active');
    showAlert('Erro ao obter taxas de conversão. Por favor, tente novamente.', 'danger');
  }
}
...
function updateRatesDisplay(rates) {
  ...
  document.getElementById('last-update-time').textContent = formatDate(lastUpdateTime);
}
