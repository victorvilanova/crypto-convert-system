...
// Importar configurações ou definir valores padrão
import { CONFIG } from '../config.js';

// Fallback para configurações caso não estejam definidas
const defaultConfig = {
  refreshRatesInterval: 60, // em segundos
  usdToBrlRate: 5.05,       // taxa de conversão padrão USD para BRL
  debugMode: false
};

// Garantir que as configurações estejam definidas
const getConfig = (key) => {
  if (typeof CONFIG === 'undefined') {
    console.warn(`CONFIG não definido, usando valor padrão para ${key}`);
...
export function initializeRatesModule() {
  ...
  updateInterval = setInterval(
    fetchCurrentRates,
    getConfig('refreshRatesInterval') * 1000
  );
...
async function fetchCurrentRates() {
  try {
    ...
    const rates = {
      BTC: data['BITSTAMP:BTCUSD'].price * getConfig('usdToBrlRate'),
      ETH: data['BITSTAMP:ETHUSD'].price * getConfig('usdToBrlRate'),
      USDT: data['BITSTAMP:USDTUSD'].price * getConfig('usdToBrlRate'),
    };
...
function updateRatesDisplay(rates) {
  ...
  if (btcRate) {
    btcRate.textContent = formatCurrency(rates.BTC);
    btcRate.style.color = rates.BTC > (currentRates.BTC || 0) ? 'green' : 'red';
    btcRate.title = `Última variação: ${(
      rates.BTC - (currentRates.BTC || 0)
    ).toFixed(2)}`;
  }
  if (ethRate) {
    ethRate.textContent = formatCurrency(rates.ETH);
    ethRate.style.color = rates.ETH > (currentRates.ETH || 0) ? 'green' : 'red';
    ethRate.title = `Última variação: ${(
      rates.ETH - (currentRates.ETH || 0)
    ).toFixed(2)}`;
  }
  if (usdtRate) {
    usdtRate.textContent = formatCurrency(rates.USDT);
    usdtRate.style.color =
      rates.USDT > (currentRates.USDT || 0) ? 'green' : 'red';
    usdtRate.title = `Última variação: ${(
      rates.USDT - (currentRates.USDT || 0)
    ).toFixed(2)}`;
  }
