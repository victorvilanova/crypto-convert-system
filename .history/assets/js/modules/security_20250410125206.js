...
function validateWalletAddress() {
  ...
  if (cryptoValue === 'BTC' && selectedNetwork === 'BTC') {
    ...
    isValid = /^[13][a-km-zA-HJ-NP-Z1-9]{25,34}$|^bc1[ac-hj-np-z02-9]{39,59}$/.test(walletValue);
    if (!isValid) {
      showValidationError(walletInput, 'Endereço Bitcoin inválido');
    }
  } else if ((cryptoValue === 'ETH' || cryptoValue === 'USDT') && selectedNetwork === 'ETH') {
  ...
  } else if ((cryptoValue === 'BNB' || cryptoValue === 'USDT') && selectedNetwork === 'BSC') {
    ...
    showValidationError(walletInput, `A rede ${selectedNetwork} não é compatível com ${cryptoValue}`);
    isValid = false;
  }
