...
function displayResult(fromCurrency, toCrypto, amount, result) {
    ...
    // Formatar valor de 1 unidade da cripto em moeda local
    const unitCryptoValue = cryptoRate.toLocaleString('pt-BR', { 
        style: 'currency', 
        currency: fromCurrency,
    ...
    let resultHTML = `
        <div class="alert alert-success">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <p class="mb-0 small">Taxa de conversão: 1 ${toCrypto} = ${unitCryptoValue}</p>
                <span class="badge bg-info">Cotação atual</span>
            </div>
    `;
    ...
    if (fromCurrency === 'BRL') {
        resultHTML += `
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <p class="mb-0"><strong>Valor de 1 ${toCrypto}:</strong> ${unitCryptoValue}</p>
                </div>
                <div>
                    <span class="badge bg-success">Valor líquido:</span>
                    <strong>${formattedFinalAmount} ${toCrypto}</strong>
                </div>
            </div>
...
function showWalletForm(fromCurrency, toCrypto, amount, finalAmount, defaultNetwork) {
    ...
    if (walletForm) {
        walletForm.addEventListener('submit', (e) => {
            ...
            const network = networkSelect ? networkSelect.value : defaultNetwork;
            ...
            console.log('Endereço validado, abrindo tela KYC...');
            // Processar a transação - direcionando para tela KYC
...
function processTransaction(fromCurrency, toCrypto, amount, finalAmount, network, walletAddress) {
    console.log('Função processTransaction chamada com sucesso!');
    console.log('Parâmetros:', {fromCurrency, toCrypto, amount, finalAmount, network, walletAddress});
    ...
    setTimeout(() => {
        ...
        // Verificar se a função showKycVerificationScreen existe
        if (typeof showKycVerificationScreen === 'function') {
            console.log('Chamando showKycVerificationScreen...');
            ...
            console.error('A função showKycVerificationScreen não está definida!');
            showError('Erro interno: não foi possível abrir a tela de verificação KYC');
        }
