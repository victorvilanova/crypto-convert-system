...
export default class ArbitrageCalculator {
  ...
  calculateTriangularArbitrage(cryptoA, cryptoB, cryptoC) {
    try {
      ...
      if (
        ...
        !this.rates[cryptoB] ||
        !this.rates[cryptoC] ||
        !this.rates[cryptoA].usd ||
        !this.rates[cryptoB].usd ||
        !this.rates[cryptoC].usd
      ) {
        return null;
      }
  ...
  findExchangeArbitrageOpportunities(exchangeRates) {
    ...
    for (const crypto of cryptos) {
      if (
        exchangeB[crypto] &&
        exchangeA[crypto]?.usd &&
        exchangeB[crypto]?.usd
      ) {
    ...
    return opportunities.sort(
      (a, b) => parseFloat(b.profitPercentage) - parseFloat(a.profitPercentage)
    );
  }
}
