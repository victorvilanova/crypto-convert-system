...
export default class CryptoModel {
  constructor() {
    ...
    this.supportedCryptos = ['bitcoin', 'ethereum', 'litecoin', 'ripple', 'cardano'];
  }
  ...
  convert(fromCrypto, toCurrency, amount) {
    if (!this.rates[fromCrypto] || !this.rates[fromCrypto][toCurrency.toLowerCase()]) {
      throw new Error(`Conversion rate not available for ${fromCrypto} to ${toCurrency}`);
    }
