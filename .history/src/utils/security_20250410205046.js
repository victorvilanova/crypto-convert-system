...
export const security = {
  ...
  validatePassword(password) {
    ...
    if (!valid) {
      const missing = [];
      if (password.length < minLength) missing.push(`pelo menos ${minLength} caracteres`);
      ...
      score += (hasUpperCase && hasLowerCase && hasNumbers && hasSpecialChars) ? 2 : 0;
    ...
    return { 
      ...
      message,
      strength,
      score: valid ? (hasUpperCase + hasLowerCase + hasNumbers + hasSpecialChars + (password.length >= minLength)) : 0
    };
  }
};