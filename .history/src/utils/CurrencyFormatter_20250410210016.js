...
export class CurrencyFormatter {
  ...
  format(value, options = {}) {
    ...
    let formattedValue = approximation ? this._getApproximateValue(value) : value;
    
    // Gerar ou obter formatador do cache
    const formatterKey = this._getFormatterKey(currency, locale, decimalPlaces, compact, showSymbol, showCode);
    let formatter = this.formatters.get(formatterKey);
    
    if (!formatter) {
      formatter = this._createFormatter(currency, locale, decimalPlaces, compact, showSymbol, showCode);
    ...
    try {
      ...
      return `${showSymbol ? this._getCurrencySymbol(currency) : ''}${formattedValue.toFixed(decimalPlaces || 2)}${showCode ? ` ${currency}` : ''}`;
    }
  }
  ...
  formatDifference(currentValue, previousValue, options = {}) {
    ...
    if (
      ...
      previousValue === null || previousValue === undefined || isNaN(previousValue) ||
      previousValue === 0
    ) {
      return 'N/A';
    }
  ...
  _formatCrypto(value, currency, formatter, options = {}) {
    ...
    if (showSymbol) {
      ...
      const currencyRegex = this._getFiatCurrencyRegex(options.locale || this.locale);
      formatted = formatted.replace(currencyRegex, this._getCryptoSymbol(currency));
    }
   ...
   * @private
   */
  _createFormatter(currency, locale, decimalPlaces, compact, showSymbol, showCode) {
    const options = {
   ...
   * @private
   */
  _getFormatterKey(currency, locale, decimalPlaces, compact, showSymbol, showCode) {
    return `${currency}_${locale}_${decimalPlaces || 'default'}_${compact ? '1' : '0'}_${showSymbol ? '1' : '0'}_${showCode ? '1' : '0'}`;
  }
