...
export class PriceAlertService {
  ...
  start() {
    ...
    if (!this.fetchPriceCallback || typeof this.fetchPriceCallback !== 'function') {
      console.error('fetchPriceCallback é necessário para iniciar o serviço de alertas');
      return false;
    }
    ...
    console.log(`Serviço de alertas de preço iniciado. Verificando a cada ${this.checkInterval / 1000} segundos.`);
  ...
  createAlert(alert) {
    ...
    console.log(`Alerta de preço criado: ${newAlert.crypto} ${newAlert.condition} ${newAlert.price} ${newAlert.targetCurrency}`);
  ...
  importFromJSON(json) {
    try {
      ...
      // Validar cada alerta
      const validAlerts = parsedAlerts.filter(alert => this._validateAlert(alert));
      
      if (validAlerts.length !== parsedAlerts.length) {
        console.warn(`${parsedAlerts.length - validAlerts.length} alertas inválidos foram ignorados.`);
      }
    ...
    if (event.key === this.storageKey) {
      try {
        if (event.newValue) {
          ...
          const maxId = Math.max(...this.alerts.map(alert => alert.id || 0), 0);
          this.nextAlertId = maxId + 1;
        } else {
  ...
  async _checkAlerts(testMode = false) {
    ...
    for (const [_, group] of alertsByPair) {
      try {
        const price = await this._fetchPrice(group.crypto, group.targetCurrency);
        ...
        console.error(`Erro ao verificar alertas para ${group.crypto}-${group.targetCurrency}:`, error);
      }
    }
  ...
  _evaluateAlert(alert, currentPrice, testMode = false) {
    ...
    switch (alert.condition) {
      ...
      case 'below':
        ...
        break;
      case 'equals':
        triggered = Math.abs(currentPrice - alert.price) < Math.max(epsilon, alert.price * 0.001);
        break;
    }
  ...
  async _fetchPrice(crypto, targetCurrency) {
    try {
      ...
      console.error(`Erro ao buscar preço para ${crypto}-${targetCurrency}:`, error);
      throw error;
    }
  }
  ...
  _triggerNotification(alert, currentPrice, testMode = false) {
    ...
    const message = `${alert.crypto} está ${conditionText[alert.condition]} ${alert.price} ${alert.targetCurrency}! Preço atual: ${currentPrice.toFixed(6)} ${alert.targetCurrency}`;
