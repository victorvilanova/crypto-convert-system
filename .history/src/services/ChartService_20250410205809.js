...
export class ChartService {
  ...
  updateChart(chartId, newData, options = {}) {
    ...
    try {
      ...
      if (this.chartLibrary === 'chartjs' && chart.instance) {
        ...
        this._createChart(chart.container, chart.type, newData, { ...chart.options, ...options });
        return true;
      }
    } catch (error) {
  ...
  _createChart(container, type, data, options) {
    try {
      ...
      if (this.chartLibrary === 'chartjs') {
        chartInstance = this._createChartJS(containerElement, type, data, options);
      } else {
        // Fallback para implementação canvas
        chartInstance = this._createCanvasChart(containerElement, type, data, options);
      }
  ...
  _createChartJS(container, type, data, options) {
    ...
    if (typeof Chart === 'undefined') {
      console.error('Chart.js não está disponível. Incluir a biblioteca ou usar outro modo.');
      return null;
    }
  ...
  _createCanvasChart(container, type, data, options) {
    ...
    switch (type) {
      ...
      case 'area':
        ...
        break;
      default:
        console.warn(`Tipo de gráfico não suportado: ${type}. Usando 'line' como fallback.`);
        this._drawLineChart(ctx, canvas.width, canvas.height, data, options);
    }
    ...
    if (options.showLegend) {
      this._drawLegend(ctx, canvas.width, canvas.height, data.datasets, options);
    }
  ...
  _drawLineChart(ctx, width, height, data, options) {
    ...
    this._drawGrid(ctx, padding, width - padding, height - padding, 5, 5, options);
    ...
    data.datasets.forEach((dataset, datasetIndex) => {
      const color = dataset.borderColor || options.colors[datasetIndex % options.colors.length];
  ...
  _drawGrid(ctx, left, right, bottom, hLines, vLines, options) {
    ...
    ctx.strokeStyle = this._getThemeColor(options.theme, 'grid', 'rgba(0,0,0,0.1)');
    ctx.lineWidth = 0.5;
  ...
  _normalizeDataForComparison(data) {
    ...
    data.datasets.forEach(dataset => {
      ...
      const normalizedValues = dataset.data.map(value => 
