...
export class ExportService {
  ...
  _exportToCSV(data, filename, options = {}) {
    ...
    // Adicionar cabeçalho
    if (includeHeader) {
      csvContent += columnDefs.map(col => this._escapeCSV(col.header || col.key)).join(delimiter) + '\n';
    }
  ...
  _exportToTXT(data, filename, options = {}) {
    ...
    // Adicionar cabeçalho
    if (includeHeader) {
      textContent += columnDefs.map(col => col.header || col.key).join(delimiter) + rowSeparator;
    }
  ...
  _exportToHTML(data, filename, options = {}) {
        ...
        <div class="container">
          ...
          <div class="table-responsive">
            <table class="data-table">
              <thead>
                <tr>
                  ${columnDefs.map(col => `<th>${col.header || col.key}</th>`).join('')}
                </tr>
              </thead>
              <tbody>
    `;
    ...
    htmlContent += `
          ...
          <div class="footer">
            <p>Gerado em: ${this._formatDate(new Date(), 'DD/MM/YYYY HH:mm:ss')}</p>
          </div>
        </div>
      </body>
      </html>
