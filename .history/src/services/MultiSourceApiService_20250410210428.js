...
export class MultiSourceApiService {
  ...
  constructor(options = {}) {
    const {
      apiKeys = {},
      apiPriority = ['coinGecko', 'coinMarketCap', 'binance', 'coinglass', 'coinapi'],
      cacheService = null,
      timeout = 10000,
      maxRetries = 2
    } = options;
  ...
  async getCurrentPrice(crypto, currency, options = {}) {
    ...
    if (compareAll) {
      ...
      return await this._getFromMultipleSources(crypto, currency, apisToTry, timeout);
    }
  }
  ...
  async getAvailableCryptos(options = {}) {
    ...
    for (const apiName of apisToTry) {
      ...
      try {
        const result = await api.getAvailableCryptos({ limit, includeMetadata });
        ...
        console.error(`Erro ao obter lista de criptomoedas de ${apiName}:`, error);
        // Continuar para próxima API
      }
    }
    
    // Se chegou aqui, nenhuma API retornou dados válidos
    throw new Error('Não foi possível obter a lista de criptomoedas de nenhuma fonte');
  }
  ...
  async getCryptoDetails(crypto, options = {}) {
    ...
    for (const apiName of apisToTry) {
      ...
      try {
        ...
        console.error(`Erro ao obter detalhes da criptomoeda ${crypto} de ${apiName}:`, error);
        // Continuar para próxima API
      }
    }
    
    // Se chegou aqui, nenhuma API retornou dados válidos
    throw new Error(`Não foi possível obter detalhes da criptomoeda ${crypto} de nenhuma fonte`);
  }
  ...
  async getMarketInfo(options = {}) {
    ...
    for (const apiName of apisToTry) {
      ...
      try {
        ...
        console.error(`Erro ao obter informações de mercado de ${apiName}:`, error);
        // Continuar para próxima API
      }
    }
    
    // Se chegou aqui, nenhuma API retornou dados válidos
    throw new Error('Não foi possível obter informações de mercado de nenhuma fonte');
  }
  ...
  updateApiKey(apiName, apiKey) {
    ...
    if (this.availableApis[apiName] && typeof this.availableApis[apiName].setApiKey === 'function') {
  ...
  updateApiPriority(newPriority) {
    ...
    for (const api of newPriority) {
      if (!this.availableApis[api]) {
        console.warn(`API ${api} na nova ordem de prioridade não está disponível`);
      }
    }
  ...
  _initializeApis() {
    ...
    // CoinMarketCap
    if (this._hasImplementation('CoinMarketCapApi') && this.apiKeys.coinMarketCap) {
  ...
  async _getFromMultipleSources(crypto, currency, apisToTry, timeout) {
    for (const apiName of apisToTry) {
      ...
      for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
        try {
          ...
          console.error(`Erro ao obter preço de ${apiName} (tentativa ${attempt + 1}/${this.maxRetries + 1}):`, error);
    ...
    throw new Error(`Não foi possível obter o preço de ${crypto} em ${currency} de nenhuma fonte`);
  }
  ...
  async _compareAllSources(crypto, currency, timeout) {
    ...
    if (validPrices.length > 0) {
      // Média
      results.averagePrice = validPrices.reduce((sum, price) => sum + price, 0) / validPrices.length;
      
      // Mediana
      const sortedPrices = [...validPrices].sort((a, b) => a - b);
      const midIndex = Math.floor(sortedPrices.length / 2);
      results.medianPrice = sortedPrices.length % 2 === 0
   ...
   * @private
   */
  async _getHistoricalFromMultipleSources(crypto, currency, apisToTry, options) {
    for (const apiName of apisToTry) {
      const api = this.availableApis[apiName];
      ...
      for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
        try {
          ...
          console.error(`Erro ao obter dados históricos de ${apiName} (tentativa ${attempt + 1}/${this.maxRetries + 1}):`, error);
    ...
    throw new Error(`Não foi possível obter dados históricos de ${crypto} em ${currency} de nenhuma fonte`);
  }
...
class CoinGeckoApi {
  ...
  async getCurrentPrice(crypto, currency) {
    ...
    const url = `${endpoint}?ids=${this._mapCryptoId(crypto)}&vs_currencies=${currency.toLowerCase()}`;
