...
export class CryptoService {
  ...
  _getMockRates() {
    ...
    const baseRates = {
      ...
      XRP: { BRL: 3.50, USD: 0.70, EUR: 0.64 }
    };
  ...
  async convert(fromCurrency, toCurrency, amount) {
    ...
    // Validar moedas
    if (!this._validateCurrency(rates, fromCurrency) || 
    ...
    const fees = this._calculateFees(fromCurrency, toCurrency, amount, convertedValue);
  ...
  _calculateFees(from, to, originalAmount, convertedAmount) {
    ...
    if (this._isCrypto(from) && !this._isCrypto(to) && convertedAmount > 35000) {
