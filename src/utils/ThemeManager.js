...
export class ThemeManager {
  ...
  setTheme(theme) {
    ...
    if (!this.availableThemes.includes(theme)) {
      console.error(
        `Tema inválido: ${theme}. Temas disponíveis: ${this.availableThemes.join(
          ', '
        )}`
      );
      return;
  ...
  updateThemeIcons() {
    ...
    themeToggles.forEach((toggle) => {
      ...
      iconElement.className = iconElement.className.replace(
        /fa-sun|fa-moon|fa-circle-half-stroke/g,
        ''
      );
      ...
      const tooltipElement =
        toggle.querySelector('[data-tooltip]') || toggle.nextElementSibling;
      ...
      if (tooltipElement && tooltipElement.hasAttribute('data-tooltip')) {
        ...
        tooltipElement.setAttribute(
          'data-tooltip',
          tooltips[currentTheme] || tooltips.light
        );
      }
    });
  }
