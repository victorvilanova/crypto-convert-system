...
function calculateAllFeesAndTaxes(fromCurrency, toCrypto, amount, convertedAmount, cryptoRate) {
    ...
    // Determinar a rede padrão para a criptomoeda
    const network = toCrypto === 'USDT' ? 'TRON' : toCrypto;
    ...
    // Calcula taxa de rede (passando a rede correta)
    const networkFee = calculateNetworkFee(toCrypto, network, fromCurrency, cryptoRate);
    ...
    return {
        ...
        totalInBRL,
        finalAmount,
        network // Incluir a rede usada para referência
    };
}
