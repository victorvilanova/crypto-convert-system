...
export default class CryptoView {
  ...
  initEventListeners(convertHandler) {
    ...
    this.fromCurrencySelect.addEventListener(
      'change',
    ...
    this.toCryptoSelect.addEventListener(
      'change',
      PerformanceUtils.debounce(() => {
        if (this.amountInput.value && parseFloat(this.amountInput.value) > 0) {
          convertHandler();
  ...
  showOfflineAlert() {
    if (this.offlineAlert) {
      ...
      alertDiv.innerHTML =
        '<strong>Você está offline!</strong> Os dados de taxas podem estar desatualizados.';
  ...
  updateRatesTable(ratesTable) {
    ...
    ratesTable.forEach((item) => {
      ...
      unitCell.textContent = `1 ${this.formatCryptoName(
        item.crypto
      )} = ${unitRate} ${item.currency}`;
  ...
  displayFavorites(favorites) {
    ...
    favorites.forEach((favorite, index) => {
      ...
      contentDiv.innerHTML = `
        <strong>${
          favorite.label ||
          `${favorite.amount} ${favorite.fromCurrency} → ${favorite.toCrypto}`
        }</strong>
      `;
